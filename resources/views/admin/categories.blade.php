            ...
            <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="icon" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="">-- Pilih Icon Kategori --</option>
                            <option value="code" {{ old('icon') == 'code' ? 'selected' : '' }}>💻 Web / Coding (code)</option>
                            <option value="dns" {{ old('icon') == 'dns' ? 'selected' : '' }}>🌐 Networking (dns)</option>
                            <option value="memory" {{ old('icon') == 'memory' ? 'selected' : '' }}>⚙️ Hardware / IoT (memory)</option>
                            <option value="visibility" {{ old('icon') == 'visibility' ? 'selected' : '' }}>👁️ Vision / AI (visibility)</option>
                            <option value="palette" {{ old('icon') == 'palette' ? 'selected' : '' }}>🎨 UI/UX Design (palette)</option>
                            <option value="database" {{ old('icon') == 'database' ? 'selected' : '' }}>🗄️ Database (database)</option>
                            <option value="terminal" {{ old('icon') == 'terminal' ? 'selected' : '' }}>⌨️ Terminal / CLI (terminal)</option>
                            <option value="science" {{ old('icon') == 'science' ? 'selected' : '' }}>🧪 Data Science (science)</option>
                            <option value="bug_report" {{ old('icon') == 'bug_report' ? 'selected' : '' }}>🐛 Testing / QA (bug_report)</option>
                        </select>
                        <p class="text-xs text-gray-500 mt-1">
                            Referensi lengkap: <a href="https://fonts.google.com/icons" target="_blank" class="text-blue-600 hover:underline">Google Material Icons</a>
                        </p>
                        @error('icon')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                        @enderror
